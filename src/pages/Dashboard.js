...
export default function Dashboard() {
  ...
  return (
    <>
      <div style={S.row}>
        {[['#34a853',counts.GREEN,'🟢 All Clear'],['#fbbc04',counts.AMBER,'🟡 Monitoring'],['#ea4335',counts.RED,'🔴 Urgent'],['#1a3a5c',counts.overdue,'⏰ Overdue'],['#666',officers.length,'👥 Total']].map(([c,n,lbl])=>(
      ...
      <div style={S.card}>
        ...
        {units.map(unit => {
          ...
          const g=uo.filter(o=>o.status==='GREEN').length, a=uo.filter(o=>o.status==='AMBER').length, r=uo.filter(o=>o.status==='RED').length;
          return (
            <div key={unit} style={{ marginBottom:12 }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:3, fontSize:13 }}>
                <strong style={{ color:'#1a3a5c' }}>{unit}</strong>
                <span><span style={{ color:'#1e7e34' }}>●{g}</span> <span style={{ color:'#b07d00' }}>●{a}</span> <span style={{ color:'#c62828' }}>●{r}</span></span>
              </div>
              ...
              <tbody>
                {recent.map((o,i)=>(
                  <tr key={o.id} style={{ background:i%2===0?'#fff':'#f8fafc' }}>
                    <td style={S.td}><strong>{o.name}</strong><div style={{ fontSize:11,color:'#888' }}>{o.rank}</div></td>
