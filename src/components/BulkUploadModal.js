...
export default function BulkUploadModal({ onClose, onSaved }) {
  ...
  // Overlay + outer shell uses flex-column so header/scroll-body/footer divide the available height
  const shellStyle = {
    background:'#fff', borderRadius:12, width:'100%', maxWidth:680, maxHeight:'88vh',
    boxShadow:'0 8px 40px rgba(0,0,0,.25)', display:'flex', flexDirection:'column', overflow:'hidden'
  };
  ...
  return (
    <div style={S.overlay} onClick={onClose}>
      <div style={shellStyle} onClick={e=>e.stopPropagation()}>
        ...
        {/* Fixed header */}
        <div style={{ padding:'24px 24px 14px 24px', borderBottom:'2px solid #c8a800', flexShrink:0 }}>
          <div style={{ fontFamily:'Georgia,serif', fontSize:17, fontWeight:'bold', color:'#1a3a5c' }}>📂 Bulk Upload Officers</div>
        </div>
        ...
        {result ? (
          ...
          <>
            ...
            <div style={{ padding:'16px 24px', overflowY:'auto', flex:1, minHeight:0 }}>
              ...
              {!rows.length && (
                <div
                  ...
                  style={{ border:'2px dashed #c8d8e8', borderRadius:10, padding:'32px 20px', textAlign:'center', cursor:'pointer', background:'#f8fafc' }}
                >
              ...
              {error && <div style={{ background:'#fce8e6', color:'#c62828', borderRadius:6, padding:'8px 12px', fontSize:13, marginTop:12 }}>{error}</div>}
              ...
              {rows.length > 0 && (
                <>
                  ...
                  <div style={{ maxHeight:280, overflowY:'auto', border:'1px solid #eee', borderRadius:7 }}>
                    <table style={{ width:'100%', borderCollapse:'collapse' }}>
                      <thead>
                        <tr>
                          {['#','Name','Unit','Email','Phone','Status','Valid?'].map(h=>(
                            <th key={h} style={{ ...S.th, padding:'7px 10px', fontSize:11, position:'sticky', top:0 }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
            ...
            <div style={{ padding:'14px 24px', borderTop:'1px solid #eee', flexShrink:0, display:'flex', gap:8, alignItems:'center', flexWrap:'wrap', background:'#fff' }}>
              {rows.length > 0 ? (
                <>
                  ...
                  {valid.length < rows.length && <span style={{ fontSize:12, color:'#888' }}>{invalid.length} row{invalid.length!==1?'s':''} will be skipped</span>}
                </>
              ) : (
                <button style={S.btnOut} onClick={onClose}>Cancel</button>
              )}
